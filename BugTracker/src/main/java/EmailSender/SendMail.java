...
public class SendMail extends EmailData {
  ...
  public static void sendEmail(String BugName, String DevName, String PATH) {
    ...
    final String from = TesterName.substring(0, spaceIndex);// extracts only 1st name - om rane only om
    ...
    properties.put("mail.smtp.port", "465");//Port 465 is used for SSL-encrypted emails.
...
//used instead of real Gmail password for security)
  ...
  public static void sendEmail(String DevName, String email, String ip, int port) {
    ...
    Session session = Session.getInstance(properties, new javax.mail.Authenticator() {
      ...
      } //“I am test.tester@gmail.com, here is my password abcxyz123. Let me send this mail.”
    ...
    try {
        ...
        //Subject and body are generated dynamically.
        message.setSubject(EmailData.getSubject());

        String portString = Integer.toString(port);
        message.setText(EmailData.getbody(DevName, ip, portString, email));
        
        Transport.send(message); //sends the email after authentication.
    } catch (MessagingException mex) {
        mex.printStackTrace();
    }
  }
